...
public class DungeonsAnimationsPillagerModel extends AnimatedGeoModel {
    @Override
    public void setLivingAnimations(IAnimatable entity, Integer uniqueID, AnimationEvent customPredicate) {
        ...
        IBone rh = this.getAnimationProcessor().getBone("rightArm");
        IBone lh = this.getAnimationProcessor().getBone("leftArm");
        ...
        if (extraData.headPitch != 0 || extraData.netHeadYaw != 0) {
            ...
            if (!t) {
                rh.setRotationX(rh.getRotationX() + ((extraData.headPitch * ((float) Math.PI / 240F))));
                lh.setRotationX(rh.getRotationX() + ((extraData.headPitch * ((float) Math.PI / 240F))));

                if (!r) {
                    lh.setRotationZ(lh.getRotationZ() - ((extraData.netHeadYaw * ((float) Math.PI / 240F))));
                    rh.setRotationZ(rh.getRotationZ() - ((extraData.netHeadYaw * ((float) Math.PI / 240F))));
                }
            }
    ...
    public ResourceLocation getTextureResource(Object object) {
        return new ResourceLocation(MODID, "textures/geo_entity/illager/pillager.png");
    }

    @Override
    public ResourceLocation getAnimationResource(Object animatable) {
        return new ResourceLocation(MODID, "animations/pillager_geo.animation.json");
    }
}
